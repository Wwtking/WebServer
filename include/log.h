...
#define SYLAR_LOG_LEVEL(logger, level) \
    if(logger->getLevel() <= level) \
        ...
        sylar::getFiberId(), time(0), sylar::Thread::getCurrThreadName())).getSS()
 ...
 * @param[...] 表示可以分开输入不同字符串作为日志流内容
 */
#define SYLAR_LOG_FMT_LEVEL(logger, level, fmt, ...) \
    if(logger->getLevel() <= level) \
        sylar::LogEventWrap(std::make_shared<sylar::LogEvent>(logger, \
        level, __FILE__, __LINE__, 0, sylar::getThreadId(), \
        sylar::getFiberId(), time(0), sylar::Thread::getCurrThreadName())).getEvent()->format(fmt, __VA_ARGS__)
...
public:
    ...
    //设置是否有自己的日志格式器
    void setHasFormatter(bool b) { m_hasFormatter = b; }

protected:
    LogLevel::Level m_level = LogLevel::DEBUG;      // 日志级别
    LogFormatter::ptr m_formatter;       //日志格式器：定义输出格式
    bool m_hasFormatter = true;    //是否有自己的日志格式器
...
public:
    ...
    //添加日志目标
    void addApender(LogAppender::ptr appender);
...
private:
    std::map<std::string, Logger::ptr> m_loggers;    //存放的所有logger
    Logger::ptr m_root;
    MutexType m_mutex;    //锁类型
};
